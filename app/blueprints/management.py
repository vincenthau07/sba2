        ...
        if SCHEMA[tname].fields[field].foreignKey:
            rtn[field] = [
                ...
                [
                    ...
                    f"""SELECT {SCHEMA[tname].fields[field].foreignKey[1]} 
                    FROM {SCHEMA[tname].fields[field].foreignKey[0]}
                    ORDER BY {SCHEMA[tname].fields[field].foreignKey[1]}"""
                    ).result
                ]
            ]
            ...
            rtn[field] = ["select", 
                          SCHEMA[tname].fields[field].options]
            
        elif SCHEMA[tname].fields[field].command:
            rtn[field] = ["select", 
                          [row[0] for row in sql(
                              *SCHEMA[tname].fields[field].command
                              ).result]
                          ]
...
def get_fields(tname):
    return [SCHEMA[tname].fields[key].displayName 
            if SCHEMA[tname].fields[key].displayName 
            else key 
            for key in SCHEMA[tname].fields]

def info(tname, tableOnly = False):
    info = sql(f"SELECT * FROM {tname} ORDER BY {SCHEMA[tname].primaryKey}",
               tupleToList = True)
...
def management(tname, permission):
    ...
    return flask.render_template('management.html', 
                                 pk_index = pk_index, 
                                 fields = fields, 
...
@blueprint.route('/management/<tname>/update', methods = ["GET"])
@verifySession(flask.session, "EDIT{0}")
def managementUpdateTable(tname, permission):
    return flask.jsonify(data = info(tname))

@blueprint.route('/management/<tname>', methods = ["POST"])
@verifySession(flask.session, "EDIT{0}")
def managementPOST2(tname, permission):
    try:
        if flask.request.form['type'] in ("update", "delete"):
            sql(f"DELETE FROM {tname} WHERE {SCHEMA[tname].primaryKey} = ?",
                flask.request.form['id'], commit = True)
        
        if flask.request.form['type'] in ("update", "insert"):
            length = len(flask.request.form.getlist("data[]"))
            placeholders = ','.join(['?'] * length)
            sql(f"INSERT INTO {tname} VALUES ({placeholders})", 
                *map(lambda i: None if i == 'None' else i,
                     flask.request.form.getlist("data[]")),
                commit = True)
