...
ScheduleTemplate.prototype.openModal = function(target) {
	...
	let event_index = $('.cd-schedule-modal[data-event^="myevent-"]').attr('data-event').substring("myevent-".length);
		$('.cd-schedule-modal__header-bg').css({'background-color': data[event_index].background_color});
	...
	if( mq == 'mobile' ) {
		...
		self.modalBodyBg.setAttribute('style', 'height:'+eventHeight+'px; width: 100px; transform: scaleY('+HeaderBgScaleY+') scaleX('+BodyBgScaleX/100+')');
...
ScheduleTemplate.prototype.loadEventContent = function(content) {
	...
	let event_index = $('.cd-schedule-modal[data-event^="myevent-"]').attr('data-event').substring("myevent-".length);
	$('.cd-schedule-modal__event-info').html('<div>'+data[event_index].content+'</div>')
	$('.cd-schedule-modal').addClass('cd-schedule-modal--content-loaded')
};
...
var scheduleTemplate = null;
var scheduleTemplateArray = [];
var resizing = null;
...
function reloadSchedule(){
	scheduleTemplate = document.getElementsByClassName('js-cd-schedule');
	scheduleTemplateArray.length = 0;
	resizing = false;
	$(".js-schedule-loaded").removeClass('js-schedule-loaded');
	$(".cd-schedule").addClass('cd-schedule--loading')
	if( scheduleTemplate.length > 0 ) { // init ScheduleTemplate objects
		for( var i = 0; i < scheduleTemplate.length; i++) {
			scheduleTemplateArray.push(new ScheduleTemplate(scheduleTemplate[i]));
		}
...
function createSchedule($schedule, interval){
	$schedule.addClass("cd-schedule cd-schedule--loading margin-top-lg margin-bottom-lg js-cd-schedule")
	code = `
	...
	while((hour<24)||((hour==24)&&(minute==0))){
		code +='<li><span>'+("0" + hour).slice(-2)+":"+("0" + minute).slice(-2)+'</span></li>'
		minute += interval;
	...
	for (let i=0; i<7; i++)
			...
			<div class="cd-schedule__top-info"><div class="date"></div><div class='space'>&nbsp-&nbsp</div><span>`+weekday[i]+`</span></div>
...
function updateSchedule(dt, column){
	$('li.cd-schedule__event').remove();
	data = dt;
	for(let i=0; i < data.length; i++){
		$(".cd-schedule__events>ul").children().eq(data[i].weekday).find("ul").append(`
			<li class="cd-schedule__event">
				<a data-start="`+data[i].start+`" data-end="`+data[i].end+`" data-event="myevent-`+i+`" style="background-color: `+data[i].background_color+`">
				  <em class="cd-schedule__name">`+data[i].title+`</em>
				</a>
			</li>
			`)
	}
	for(let i=0; i < column.length; i++){
		$(".cd-schedule__events>ul").children().eq(i).find('.date').html(column[i])
	}
	reloadSchedule();
}
